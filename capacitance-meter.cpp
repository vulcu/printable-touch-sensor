...
void CMeter::loop(void) {
  ...
  if (val < 1000) {
    ...
    this->result.capacitance = (float)val * this->IN_CAP_TO_GND / (float)(this->MAX_ADC_VALUE - val);

    Serial.print(F("Capacitance Value = "));
    Serial.print(this->result.capacitance, 3);
    Serial.print(F(" pF ("));
    Serial.print(val);
    Serial.println(F(") "));
  }
    ...
    Serial.print(F("Capacitance Value = "));
    if (this->result.capacitance > 1000.0) {
      Serial.print(this->result.capacitance / 1000.0, 2);
      Serial.print(F(" uF"));
    }
    else {
      Serial.print(this->result.capacitance, 2);
      Serial.print(F(" nF"));
    }

    Serial.print(F(" ("));
    Serial.print(this->result.isLargeCapValue ? F("Normal") : F("HighVal"));
    Serial.print(F(", t= "));
    Serial.print(this->result.chargeTime);
    Serial.print(F(" us, ADC= "));
    Serial.print(this->result.adc);
    Serial.println(F(")"));
  }
}
